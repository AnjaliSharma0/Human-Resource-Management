...
export default function JobRequisitionAdmin() {
  ...
  const updateStatus = async (id: number, newStatus: string, currentStatus: string) => {

    // 🔒 prevent duplicate action
    if (currentStatus === "Approved" || currentStatus === "Rejected") {
      return toast.error("Action already taken");
    }

    try {
      await api.patch(`/job-requisition/${id}/status`, { status: newStatus });
              ...
              return (
                <tr key={r.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-3 flex gap-2 items-center">
                    ...
                    <button
                      onClick={() =>
                        updateStatus(r.id, "Approved", r.status)
                      }
                      disabled={isActionTaken}
                    ...
                    <button
                      onClick={() =>
                        updateStatus(r.id, "Rejected", r.status)
                      }
                      disabled={isActionTaken}
                      className={`px-3 py-1 rounded text-sm text-white transition
                        ${isActionTaken
