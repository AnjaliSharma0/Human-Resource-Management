...
//  const updateStatus = async (id: number, status: "pending" | "in-progress" | "completed") => {
//   try {
...
//   const status = value as "pending" | "in-progress" | "completed";
...
//                   ([skills, status]) => (
//                     <tr key={skills} className="border-t">
    ...
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen">
      ...
      {activeTab === "courses" && role === "admin" && (
        <>
          <div className="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <div />
            <button
              className="bg-indigo-600 text-white px-4 py-2 rounded shadow hover:bg-indigo-700 transition w-full sm:w-auto"
              onClick={() => {
                setEditingCourse(null);
                setModalOpen(true);
              }}
      ...
      {activeTab === "enrollments" && role === "admin" && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {enrollments.map((e) => (
              <div key={e.id} className="bg-white p-5 rounded-2xl shadow flex flex-col">
                <h4 className="font-semibold text-gray-800">
                  {e.employee.firstName} {e.employee.lastName}
                </h4>
                <p className="text-gray-500 text-sm mt-1">{e.course.title}</p>
                <select
                  value={e.status}
                  onChange={(event) => handleStatusChange(e.id, event.target.value)}
                  className="border mt-3 p-2 rounded w-full text-sm focus:ring-1 focus:ring-indigo-500"
                >
                  <option value="pending">Pending</option>
                  <option value="in-progress">In Progress</option>
                  <option value="completed">Completed</option>
      ...
      {activeTab === "my-courses" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {courses.map((course) => (
            <div key={course.id} className="bg-white p-5 rounded-2xl shadow flex flex-col">
      ...
      {activeTab === "skills" && (
        <>
          <div className="hidden sm:block bg-white rounded-2xl shadow overflow-x-auto">
            <table className="w-full">
              <tbody>
                {Object.entries(skillMatrix[0]?.skills || {}).map(([skill, status]) => (
                  <tr key={skill} className="border-t hover:bg-gray-50 transition">
                    <td className="p-3 font-medium">{skill}</td>
                    <td className="p-3 text-indigo-600">{String(status)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="sm:hidden space-y-3">
            {Object.entries(skillMatrix[0]?.skills || {}).map(([skill, status]) => (
              <div key={skill} className="bg-white p-4 rounded-2xl shadow">
                <p className="font-semibold text-gray-800">{skill}</p>
                <p className="text-sm text-indigo-600 mt-1">{String(status)}</p>
              </div>
            ))}
          </div>
        </>
      ...
      {activeTab === "employees" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {employees.map((emp) => (
            <div key={emp.id} className="bg-white p-5 rounded-2xl shadow">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl w-[90%] max-w-sm shadow-lg">
            <h3 className="font-semibold mb-3 text-gray-800">Confirm Delete</h3>
            <div className="flex justify-end gap-2">
              <button className="px-3 py-1 rounded border hover:bg-gray-100" onClick={() => setDeleteId(null)}>Cancel</button>
              <button
